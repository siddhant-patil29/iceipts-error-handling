'use strict';
/**
 *  This module is used to define service for Warrantys model 
 *  @module Warrantys-services
 *  @author Shravan Kothapalli
 *  @version 1.0.0
 */
var _ = require('underscore');
var warrantyDao = require('../daos/warranty-dao');
...
module.exports = {
    ...
    getWarranty: function (req) {
        return new Promise(function (resolve, reject) {
            warrantyDao.getWarranty({ id: req.params.id, isDeleted: 0 }).then(function (result) {
    ...
    /**
     * Controller function for get Warrantys data
     */
    
    getWarranties: function (req) {
        return new Promise(function (resolve, reject) {
            warrantyDao.getWarranties({ isDeleted: 0 }).then(function (result) {
                logger.debug('success ',result);
                return resolve(util.responseUtil(null, result, responseConstant.SUCCESS));
            }, function(err){
                logger.error(err);
    ...
    /**
     * Controller function for insert Warrantys data
     */
    addWarranty: function (req) {
        return new Promise(function (resolve, reject) {
            ...
            warrantyDao.addWarranty(insertObj).then(function (result,err) {
     ...
     */
    updateWarranty: function (req) {
        return new Promise(function (resolve, reject) {
            var insertObj = req.body;
            //check roleId exist or not: its validated automatically as its foreign key
            warrantyDao.updateWarranty(insertObj).then(function (result) {
                logger.debug('success ',result);
                return resolve(util.responseUtil(null, result, responseConstant.SUCCESS));
            }, function (err) {
                logger.error('error ',err);
    ...
    deleteWarranty: function (req) {
        return new Promise(function (resolve, reject) {
            ...
            warrantyDao.getWarranty({ id: req.params.id, isDeleted: 0 }).then(function (result) {
                    ...
                    warrantyDao.deleteWarranty(updateObj, { id: req.params.id, isDeleted: 0 }).then(function (result) {
                        logger.debug('success ',result);
